/*
 * Execute Sparql query code (RDF data) 
 * 
 */
function sparqlPrefectureQuery(sparqlCodeId, queryCode, endpoint, callback, isDebug) {
   ...
   if(xmlhttp.readyState == 4) {
	    if(xmlhttp.status == 200) {
	    		if(isDebug) {
	    			callback(xmlhttp.responseText, sparqlCodeId);
	    		} else { 
